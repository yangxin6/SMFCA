...
from pointcept.utils.DFSP.utils import seg_items
...
def post_process(clusters_np, semantic_pred_np, xyz_np, k=20, npoint_th=100, eps=0.05, min_points=100):
    ...
    clusters_np = np.delete(clusters_np, new_zero_clusters, axis=0)
    # 未识别点云的索引
    ...
    new_clusters, new_semantic_pred, cluster_semantic_id = assign_new_clusters(xyz_np, clusters_np, semantic_pred_np, res, no_clusters_idx, npoint_th=npoint_th)
    # except Exception as e:
    #     print(f"An error occurred: {e}")
    #     return None, None, None, None
    return new_clusters, new_semantic_pred, cluster_semantic_id


def post_process2(clusters_np, semantic_pred_np, xyz_np, k=20, npoint_th=100, eps=0.01, min_points=50, stem_threshold=0.1):
    stem_idx = np.where(semantic_pred_np == 0)
    # 改进的stem_c计算方法
    stem_c = -1
    max_stem_ratio = -1
    ...
    res = np.ones(no_clusters_points.shape[0]) * -1
    new_clusters, new_semantic_pred, cluster_semantic_id = assign_new_clusters2(xyz_np, clusters_np, semantic_pred_np, res, no_clusters_idx, npoint_th=npoint_th)
    # except Exception as e:
    #     print(f"An error occurred: {e}")
    #     return None, None, None, None
    return new_clusters, new_semantic_pred, cluster_semantic_id
...
def assign_new_clusters2(xyz, clusters_np, semantic_pred_np, res, no_clusters_idx, k=5, npoint_th=100):
    res = res.astype(int)
    ...
    return clusters, semantic_pred, cluster_semantic_id




def assign_new_clusters(xyz, clusters_np, semantic_pred_np, res, no_clusters_idx, k=5, npoint_th=100):
    res = res.astype(int)
    clusters = clusters_np.copy()
    semantic_pred = semantic_pred_np.copy()
    kdt = KDTree(xyz, metric='euclidean')
    ...
    for i in range(max(res) + 1):
        ...
        if len(i_nc[0]) > npoint_th:
            new_res_label[i_nc] = 1000 + new_index
            new_index += 1
