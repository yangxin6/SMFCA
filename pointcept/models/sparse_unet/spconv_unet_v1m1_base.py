...
class SpUNetBase(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        x = skips.pop(-1)
        # print("=======================vis")
        # fn = input_dict['name'][0]
        # # fn = "Maize_1-111"
        # cat = fn.split('_')[0]
        # from pointcept.utils.visualization import visualize_heat_high_o3d
        # save_dir = f"/home/yangxin/Projects/plant_seg/images/crops3d/maize_ablation/Attn_heat_map_pcd2/cls/{cat}/"
        # # save_dir = f"/home/yangxin/Projects/plant_seg/images/crops3d/maize_ablation/Attn_heat_map_pcd2/seg/"
        # save_name = f"SPUNet_{fn}.ply"
        # visualize_heat_high_o3d(x=x, coord=input_dict["coord"], save_scalar_ply=f"{save_dir}{save_name}",
        #                         show=False)
        ...
        if self.encoder_mode:               # ★ 新增分支
            # enc_feats 已按深→浅，需与 decoder_feats 对齐可自行 reverse
            return x.features, enc_feats + [decoder_feats[-1]]
        elif self.decoder_mode:
            return x.features, decoder_feats
        else:
            return x.features
