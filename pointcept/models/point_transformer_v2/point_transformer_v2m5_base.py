...
class GroupedVectorAttention(nn.Module):
    ...
    def forward(self, feat, coord, reference_index, attn_mask=None):
        query, key, value = (
            self.linear_q(feat),
            self.linear_k(feat),
            self.linear_v(feat),
        ...
        # 这个地方 relation_qk weight_encoding 谁先谁后
        if attn_mask is not None:
            attn_mask_inf = torch.where(attn_mask, torch.tensor(1e-4).to(attn_mask.device),
                                        torch.tensor(0.0).to(attn_mask.device))
            attn_mask_g = pointops.grouping(reference_index, attn_mask_inf, coord, with_xyz=False)
            relation_qk += attn_mask_g
...
class Block(nn.Module):
    ...
    def forward(self, points, reference_index, attn_mask=None):
        coord, feat, offset = points
        identity = feat
        feat = self.act(self.norm1(self.fc1(feat)))
        feat = (
            self.attn(feat, coord, reference_index, attn_mask=attn_mask)
            if not self.enable_checkpoint
            else checkpoint(self.attn, feat, coord, reference_index, attn_mask=attn_mask)
        )
...
class MaskBlockSequence(nn.Module):
    def __init__(
            self,
            depth,
            embed_channels,
        ...
        super(MaskBlockSequence, self).__init__()
    ...
    def forward(self, points):
        ...
        for block in self.blocks:
            # Mask
            coord, feat, offset = points
            mask_features = self.mask_embed_head(feat)
            instance_attn = mask_features.sigmoid() < 0.5
            mm = [points, mask_features]
            points = block(points, reference_index, attn_mask=instance_attn)
        return points, mm
...
class PointTransformerV2(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        for i in reversed(range(self.num_stages)):
            ...
            points, mm = self.dec_stages[i](points, skip_points, cluster)
            aux_mask_outputs.append([mm, cluster])

        # last loss
        coord, feat, offset = points
        out_mask_feat = self.mask_embed_head(feat)

        return_dict = dict(
            out_masks=out_mask_feat,
        )
        out_dict = self.loss(aux_mask_outputs)
        return_dict.update(out_dict)

        if not self.training:
            out_dict2 = self.pred_feat(out_mask_feat, offset)
            return_dict.update(out_dict2)

        return return_dict

    def loss(self, aux_mask_outputs):
        out_dict = dict()
        # aux_output
        aux_out_feat = []
        for item in aux_mask_outputs:
            [layer_points, layer_mask_feat], cluster = item
            aux_out_feat.append(
                dict(layer_points=layer_points, layer_mask_feat=layer_mask_feat, cluster=cluster))

        out_dict['aux_outputs'] = aux_out_feat
        return out_dict

    def pred_feat(self, pred_mask, offset):
        ...
        for b in range(batch_size):
            ...
            b_mask_pred = pred_mask[b_point_mask].T
            ...
            # score_thr
            b_score_mask = b_mask_scores > self.score_thr
            ...
            b_classes = torch.zeros(b_mask_pred.size(0), dtype=torch.int).to(pred_mask.device)
            b_scores = torch.ones(b_mask_pred.size(0), dtype=torch.int).to(pred_mask.device)
            classes.append(b_classes)
            scores.append(b_scores)
            masks.append(b_mask_pred)
        return dict(
            pred_classes=torch.cat(classes).cpu().numpy(),
