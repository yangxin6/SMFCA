...
class DynamicGridPool(nn.Module):
    """
    Dynamic Partition-based Pooling (FPS + kNN)

    用 FPS 进行下采样，并通过 kNN 聚合邻域特征
    返回下采样后的 (coord, feat, offset) 和原始点到新点的 cluster 映射。
    """
    ...
    def __init__(self,
        ...
        self.k = k
        # 特征投影
        self.fc_proj = nn.Linear(in_channels, out_channels, bias=bias)
    ...
    def forward(self, points):
        ...
        # 投影 + 归一化 + 激活
        feat_proj = self.act(self.norm(self.fc_proj(feat)))
        # 1) FPS 采样中心点
        fps_idx = fps(coord, batch, ratio=self.sample_ratio, random_start=False)
        centroids = coord[fps_idx]        # (M,3)
        c_batch = batch[fps_idx]        # (M,)
        _, cluster = knn(
            x=centroids,
            y=coord,
            k=1,
        ...
        agg_feats = scatter(feat_proj, cluster, dim=0, reduce='max')  # (M, C_out)

        # 6) 构建新的 offset
        new_offset = batch2offset(c_batch)

        return [centroids, agg_feats, new_offset], cluster
...
@MODELS.register_module("SACLA-PT-v2m0")
class PointTransformerV2(nn.Module):
    def __init__(
        ...
        assert self.num_stages == len(sample_ratios)
