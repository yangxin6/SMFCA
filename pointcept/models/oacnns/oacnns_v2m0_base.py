...
#         self.proj = nn.Linear(hidden_channels, in_channels)

#     def forward(self, x, cluster):
#         q = F.relu(self.q(x))
#         k = scatter(F.relu(self.k(x)), cluster, dim=0, reduce='mean')[cluster]
#         v = scatter(self.v(x), cluster, dim=0, reduce='mean')[cluster]
...
# class EnhancedLinearAttention(nn.Module):
#     def __init__(self, in_channels, hidden_channels, indice_key=None, norm_fn=None, bias=False):
#         super().__init__()
#         self.q = nn.Linear(in_channels, hidden_channels)
#         self.k = nn.Linear(in_channels, hidden_channels)
#         self.v = nn.Linear(in_channels, hidden_channels)
#
#         self.local_conv = spconv.SparseSequential(
#             spconv.SubMConv3d(
#                 in_channels, hidden_channels, kernel_size=3, padding=1,
#                 indice_key=indice_key, bias=bias
#             ),
#             norm_fn(hidden_channels),
#             nn.ReLU(),
#         )
#
#         self.fusion = nn.Sequential(
#             nn.Linear(2 * hidden_channels, in_channels),
#             norm_fn(in_channels),
#             nn.ReLU(),
#         )
#
#     def forward(self, x, cluster):
#         feat = x.features
#         q = F.relu(self.q(feat))                           # [N, C']
#         k = scatter(F.relu(self.k(feat)), cluster, dim=0, reduce='mean')[cluster]
#         v = scatter(self.v(feat), cluster, dim=0, reduce='mean')[cluster]
#         attn = (q * k).sum(dim=-1, keepdim=True) / (q.size(-1) ** 0.5)
#         attn = torch.softmax(attn, dim=0)
#         attn_out = attn * v                                # [N, C']
#
#         local_feat = self.local_conv(x).features # [N, C']
#
#         concat_feat = torch.cat([attn_out, local_feat], dim=1)  # [N, 2*C']
#         return self.fusion(concat_feat)                    # [N, in_channels]
...
#             EnhancedLinearAttention(embed_channels, embed_channels // 2, indice_key=indice_key, norm_fn=norm_fn, bias=bias)
...
#         feat = x.features
#         feats = [branch(x, cluster) for branch, cluster in zip(self.attn_branches, clusters)]
#         adp = torch.softmax(self.adaptive(feat), dim=1)
#         feats = torch.stack(feats, dim=1)  # [N, num_scales, C]
#         feats = torch.sum(adp.unsqueeze(-1) * feats, dim=1)  # [N, C]
#
#         proj_feat = self.proj(feat)
#         fuse_feat = self.fuse(torch.cat([proj_feat, feats], dim=1)) + feat
#         x = x.replace_feature(fuse_feat)
#         x = self.voxel_block(x)
#         x = x.replace_feature(self.act(x.features + fuse_feat))
#         return x
...
@MODELS.register_module("OACNNs-v2m0")
