...
class DownBlock(nn.Module):
    ...
    def dynamic_voxel(self, x, size):
        coord = x.indices[:, 1:].float()
        batch = x.indices[:, 0]
        feat = x.features
        dv_feat = self.dv(feat)
        mask = dv_feat > 0.5

        cluster = torch.full_like(batch, -1, dtype=torch.long)
        if mask.sum() > 0:
            # 仅对叶子区域的点进行体素化
            cluster[mask] = voxel_grid(pos=coord[mask], size=size, batch=batch[mask])

        return cluster

    def downsample_segment(self, segment, indices, downsample_indices):
        """
        根据下采样后的稀疏张量的索引生成对应的 segment1。
        Args:
            segment: Tensor of shape [N], 原始的点的分割标签。
            indices: Tensor of shape [N, 4], 原始稀疏张量的索引 (batch, z, y, x)。
            downsample_indices: Tensor of shape [M, 4], 下采样后的稀疏张量的索引。
        Returns:
            segment1: Tensor of shape [M], 下采样后的分割标签。
        """
        # 创建映射关系
        voxel_dict = defaultdict(list)
        for idx, down_idx in enumerate(downsample_indices.tolist()):
            voxel_dict[tuple(down_idx)].append(idx)

        # 多数投票
        segment1 = []
        for down_idx in downsample_indices.tolist():
            down_idx_tuple = tuple(down_idx)
            if down_idx_tuple in voxel_dict:
                # 找到映射到该体素的原始点
                original_indices = voxel_dict[down_idx_tuple]
                original_segments = segment[original_indices]
                # 计算多数投票
                most_common_segment = torch.mode(original_segments).values
                segment1.append(most_common_segment)
            else:
                # 若无点映射，赋默认值 (通常是背景类 0)
                segment1.append(0)

        return torch.tensor(segment1, dtype=segment.dtype, device=segment.device)

    def forward(self, x_in, segment):
        x = self.down(x_in)
        coord = x.indices[:, 1:].float()
        batch = x.indices[:, 0]

        down_segment = self.downsample_segment(segment, x_in.indices, x.indices)
        mask = down_segment != 3

        # feat = x.features
        # dv_feat = self.dv(feat).squeeze(-1)
        # mask = dv_feat > 0.5
        ...
        for grid_size in self.point_grid_size:
            cluster = torch.full_like(batch, -1, dtype=torch.long)
            if mask.sum() > 0:
                cluster[mask] = voxel_grid(pos=coord[mask], size=grid_size, batch=batch[mask])
...
@MODELS.register_module("OACNNs-v2m1")
