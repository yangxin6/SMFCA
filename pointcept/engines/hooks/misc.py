...
@HOOKS.register_module()
class CheckpointLoader0(HookBase):
...
class CheckpointLoader(HookBase):
    def __init__(self, keywords="", replacement=None, strict=False, target_module_name=None):
        self.keywords = keywords
        self.replacement = replacement if replacement is not None else keywords
        self.strict = strict
        self.target_module_name = target_module_name  # 新增：只加载该子模块（如 'tiny_backbone1'）
    ...
    def before_train(self):
        ...
        if self.trainer.cfg.weight and os.path.isfile(self.trainer.cfg.weight):
            ...
            # ------------------ 只加载某个子模块 ------------------
            if self.target_module_name is not None:
                submodule = getattr(self.trainer.model, self.target_module_name)
                possible_prefixes = [f"module.{self.target_module_name}.", f"{self.target_module_name}."]
                filtered = {}
                for k, v in state_dict.items():
                    for prefix in possible_prefixes:
                        if k.startswith(prefix):
                            filtered[k[len(prefix):]] = v
                missing, unexpected = submodule.load_state_dict(filtered, strict=self.strict)
                self.trainer.logger.info(
                    f"[{self.target_module_name}] missing_keys: {missing} | unexpected_keys: {unexpected}"
                )
            else:
                # ------------------ 旧逻辑：全模型加载 ------------------
                weight = OrderedDict()
            ...
            # resume 优化器、调度器等只适用于全模型resume
            if self.trainer.cfg.resume and self.target_module_name is None:
...
@HOOKS.register_module()
class FreezeModelWeights(HookBase):
    def __init__(self, modules_to_freeze):
        """
        冻结指定模块的权重。

        参数:
            modules_to_freeze (list[str]): 要冻结的模块名称列表（以字符串形式传入，例如 'backbone' 或 'seg_head'）。
        """
        self.modules_to_freeze = modules_to_freeze

    def before_train(self):
        self.trainer.logger.info("=> Freezing specified model weights...")
        for module_name in self.modules_to_freeze:
            module = getattr(self.trainer.model.backbone, module_name, None)
            if module is None:
                self.trainer.logger.warning(f"Module '{module_name}' not found in model.")
                continue

            # 冻结权重但保留计算图
            for param in module.parameters():
                param.requires_grad = False
            self.trainer.logger.info(f"Module '{module_name}' has been frozen.")
